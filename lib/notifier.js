...
const { notifyDiscord } = require("./discord-bot");
...
function notifyIfChanged(businesses) {
  ...
  const embedMessage = createDiscordEmbed(filteredBusinesses);
...
function createDiscordEmbed(businesses) {
  let webhook_request = {
  ...
  webhook_request.embeds =  businesses
    .map(
      (business) =>
      ({
        title: business.display_name,
        color: 1376000,
        url: "https://share.toogoodtogo.com/item/"+business.item.item_id,
        fields : [
          {
            name: "Price",
            value: String(business.item.price_including_taxes.minor_units/100),
            inline: true
          },
          {
            "name": "Quantity",
            "value": String(business.items_available),
            "inline": true
          },
          {
            "name": "Pickup",
            "value": formatInterval(business),
            "inline": true
          }
        ]
