...
const { mqttPublish } = require("./mqtt");
...
function notifyIfChanged(businesses) {
  ...
  if (filteredBusinesses.length > 0) {
    ...
    if (config.get("notifications.mqtt.enabled")) {
      createMqttMessages(filteredBusinesses)
        .forEach((message) => mqttPublish(message));
    }
  }
...
function createMqttMessages(businesses) {
  return businesses
    .map((business) => `{"name": "${business.display_name}", "quantity": ${business.items_available}, "pickup": "${formatInterval(business)}"}`)
}
