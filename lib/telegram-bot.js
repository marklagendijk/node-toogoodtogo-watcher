...
const botCommands = [
  ...
  {
    command: "filter",
    description: "Activate or disable filter, but leave list alone."
  },
  {
    command: "filter_set",
    description: "List of general words to add to filter, with a space after it."
  },
  {
    command: "filter_unset",
    description: "List of general words to remove from filter, with a space after it."
  },
  {
    command: "filter_get",
...
function filterOnCommand(context) {
  const activate = activateOption("wordFilter");
  context.reply(
      "✅ Favorites will be filter depends on the word list."
  );
}

function filterOffCommand(context) {
  const activate = disableOption("wordFilter");
  context.reply(
      "❌ Favorites will not be filter depends on the word list."
  );
}

function filterClearCommand(context) {
  setCustomOption("wordList", []);
  context.reply(
      "✅ Words list has been cleared."
  );
}
...
function filterSetCommand(context) {
  const oldWords = context.message.split(' ');
  const config = getConfig();
  const newList = config.messageFilter.wordsList.filter(word => !oldWords.include(word));
  setCustomOption("wordList", newList);
  context.reply(
      `✅ ${newList}
      `
  );
...
function filterUnsetCommand(context) {
  const newWords = context.message.split(' ');
  const config = getConfig();
