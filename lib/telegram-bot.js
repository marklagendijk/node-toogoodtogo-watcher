...
function createBot() {
  ...
  bot.command(getCommand("start"), startCommand);
  bot.command(getCommand("stop"), stopCommand);
  bot.command(getCommand("config"), configCommand);
  bot.command(getCommand("showUnchanged"), showUnchangedCommand);
  bot.command(getCommand("showDecrease"), showDecreaseCommand);
  bot.command(getCommand("showDecreaseToZero"), showDecreaseToZeroCommand);
  bot.command(getCommand("showIncrease"), showIncreaseCommand);
  bot.command(getCommand("showIncreaseFromZero"), showIncreaseFromZeroCommand);
  bot.launch();
  return bot;
}
...
function startCommand(context) {
  addChat(context);
  context.telegram.setMyCommands(getCommands());
  context
    .reply(
...
function getCommands() {
  const configCommands = config.get("notifications.telegram.commands");
  let commands = [];
  for (let key in configCommands) {
    if (Object.prototype.hasOwnProperty.call(configCommands, key))
      commands.push(configCommands[key]);
  }
  return commands;
}
...
function getCommand(key) {
  const configCommands = config.get("notifications.telegram.commands");
  return configCommands[key].command;
}
