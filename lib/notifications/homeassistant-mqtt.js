...
function publishGeneralConfig(mqttConnection) {
  mqttConnection.publish("homeassistant/text/toogoodtogo-watcher/general/config", "");
  ...
  mqttConnection.publish("homeassistant/sensor/togoodtogo-watcher/message/config", "");
  mqttConnection.publish(
    "homeassistant/sensor/togoodtogo-watcher/message/config",
    JSON.stringify({
      name: "Message",
...
function publishLoginButton(login_started) {
  ...
  mqttConnection.publish(
    ...
    JSON.stringify({
      ...
      payload_press: JSON.stringify({ command: login_started ? "continue_login" : "login" }),
