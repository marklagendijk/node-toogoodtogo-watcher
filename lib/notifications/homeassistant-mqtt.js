...
function getMqttConnection() {
    if (!mqttConn)
        mqttConn = mqtt.connect(config.get('notifications.mqtt.url'), {
...
function publishStock(deviceName, uniqueId, state, extraAttributes = {}) {
    ...
    if (!entitiesCache[uniqueId]) {
        ...
        mqttClient.publish(`homeassistant/binary_sensor/toogoodtogo-watcher/${uniqueId}/config`, JSON.stringify({
            "name": deviceName,
